...
import { CalendarDays, ChartNoAxesCombined, ChevronLeft, ChevronRight, CircleDollarSign, Clapperboard, LogOut, Plus, Search, Trash2, Users, X as CloseIcon } from "lucide-react";
...
type PaymentStatus = "paid" | "partial" | "waiting" | "not_due" | "no_fee";
type Publication = { id: string; sponsor_id: string | null; sponsor_name?: string | null; platform: string; format: string; slot_id: string | null; title: string; planned_date: string; published_date: string | null; status: "planned" | "published" | "cancelled"; url: string | null; fee_minor: number; currency: Currency; notes: string; paid_minor?: number; outstanding_minor?: number; payment_status?: PaymentStatus };
type Transaction = { id: string; sponsor_id: string | null; sponsor_name?: string | null; publication_id: string | null; publication_title?: string | null; kind: "income" | "expense" | "credit"; amount_minor: number; currency: Currency; applied_minor: number | null; applied_currency: Currency | null; occurred_on: string; note: string };
...
const remove = <T,>(path: string) => api<T>(path, { method: "DELETE" });
...
function App() {
  ...
  const [presetTransactionPublication, setPresetTransactionPublication] = useState<Publication | null>(null);
  ...
  const openTransaction = (item?: Transaction, publication?: Publication) => { setEditingTransaction(item || null); setPresetTransactionPublication(publication || null); setError(""); setModal("transaction"); };
  const closeModal = () => { setModal(null); setEditingPublication(null); setEditingTransaction(null); setPresetTransactionPublication(null); setPresetSlot(null); setError(""); };
  const afterSave = async () => { closeModal(); setRevision((n) => n + 1); await refresh(); };
  const deletePublication = async (item: Publication) => {
    if (!window.confirm(`“${item.title}” içeriği kaldırılsın mı? İlişkili finans hareketleri korunur.`)) return;
    setBusy(true); setError("");
    try { await remove(`/publications/${item.id}`); await afterSave(); }
    catch (e) { setError((e as Error).message); }
    finally { setBusy(false); }
  };
  const deleteSponsor = async (sponsor: Sponsor) => {
    if (!window.confirm(`“${sponsor.name}” sponsor listesinden kaldırılsın mı? Geçmiş içerikler ve finans hareketleri korunur.`)) return;
    setBusy(true); setError("");
    try { await remove(`/sponsors/${sponsor.id}`); setSelectedSponsorId(null); await afterSave(); }
    catch (e) { setError((e as Error).message); }
    finally { setBusy(false); }
  };
  const deleteTransaction = async (item: Transaction) => {
    const subject = item.publication_title ? `“${item.publication_title}” için ` : "";
    if (!window.confirm(`${subject}${money(item.amount_minor, item.currency)} tutarındaki ${transactionLabel(item.kind).toLocaleLowerCase("tr")} silinsin mi? İlgili video ve finans toplamları yeniden hesaplanır.`)) return;
    setBusy(true); setError("");
    try { await remove(`/transactions/${item.id}`); await afterSave(); }
    catch (e) { setError((e as Error).message); }
    finally { setBusy(false); }
  };
  ...
  return <div className="app-shell">
    ...
    {modal && <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) closeModal(); }}><div className="modal-panel"><div className="modal-head"><div><h2>{modal === "sponsor" ? currentSponsor ? "Sponsoru düzenle" : "Yeni sponsor" : modal === "publication" ? editingPublication ? "İçeriği düzenle" : "Yeni içerik" : editingTransaction ? "İşlemi düzenle" : "Yeni işlem"}</h2></div><button className="icon-button" onClick={closeModal}><CloseIcon size={21} /></button></div>
      ...
      {modal === "publication" && <PublicationForm sponsors={sponsors} slot={presetSlot} item={editingPublication} defaultSponsor={page === "sponsors" ? selectedSponsorId : null} busy={busy} onDelete={editingPublication ? () => void deletePublication(editingPublication) : undefined} onSubmit={async (data) => { setBusy(true); setError(""); try { if (editingPublication) await patch(`/publications/${editingPublication.id}`, data); else await post("/publications", data); await afterSave(); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }} />}
      {modal === "transaction" && <TransactionForm sponsors={sponsors} item={editingTransaction} defaultSponsor={page === "sponsors" ? selectedSponsorId : null} defaultPublication={presetTransactionPublication} rate={rate} busy={busy} onDelete={editingTransaction ? () => void deleteTransaction(editingTransaction) : undefined} onSubmit={async (data) => { setBusy(true); setError(""); try { if (editingTransaction) await patch(`/transactions/${editingTransaction.id}`, data); else await post("/transactions", data); await afterSave(); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }} />}
    </div></div>}
  </div>;
}
...
function SponsorDetail({ sponsor, rate, displayCurrency, onBack, onEdit, onDelete, onAddPublication, onAddSlotPublication, onAddTransaction, onAddPayment, onEditPublication, onEditTransaction }: { sponsor: Sponsor; rate: Rate; displayCurrency: Currency; onBack: () => void; onEdit: () => void; onDelete: () => void; onAddPublication: () => void; onAddSlotPublication: (slot: Slot) => void; onAddTransaction: () => void; onAddPayment: (publication: Publication) => void; onEditPublication: (item: Publication) => void; onEditTransaction: (item: Transaction) => void }) {
  ...
  return <div className="page-stack"><div className="detail-heading"><button className="back-link" onClick={onBack}><ChevronLeft size={16} /> Sponsorlar</button><div className="detail-title"><SponsorLogo sponsor={sponsor} /><div><h2>{sponsor.name}</h2>{sponsor.website_url && <a href={sponsor.website_url} target="_blank" rel="noreferrer">Web sitesini aç ↗</a>}</div><div className="detail-actions"><Button variant="outline" size="sm" onClick={onEdit}>Düzenle</Button><Button variant="outline" size="sm" className="danger-button" onClick={onDelete}><Trash2 size={15} /> Kaldır</Button></div></div>{sponsor.notes && <p className="sponsor-note">{sponsor.notes}</p>}</div>
    ...
    <Card className="content-card"><CardHeader><div className="card-header-row"><div><CardTitle>Sponsorlu yayınlar</CardTitle></div><Button size="sm" onClick={onAddPublication}><Plus size={16} /> İçerik ekle</Button></div></CardHeader><CardContent>{detail?.publications.filter((p) => p.status !== "cancelled").length ? detail.publications.filter((p) => p.status !== "cancelled").map((p) => <div className="detail-list-row publication-payment-row" key={p.id}><button className="publication-main" onClick={() => onEditPublication(p)}><div><strong>{p.title}</strong><small>{p.platform} · {trDate(p.planned_date)} {p.url ? "· Bağlantı kayıtlı" : ""}</small></div><span className={`status-pill ${p.status}`}>{p.status === "published" ? "Yayında" : "Planlandı"}</span><b>{money(p.fee_minor, p.currency)}</b><ChevronRight size={16} /></button><div className="payment-state"><span className={`payment-pill ${p.payment_status || "not_due"}`}>{paymentLabel(p.payment_status)}</span>{p.fee_minor > 0 && p.payment_status !== "paid" && <button type="button" onClick={() => onAddPayment(p)}>Ödeme ekle</button>}{p.payment_status === "partial" && <small>{money(p.outstanding_minor || 0, p.currency)} kaldı</small>}</div></div>) : <Empty text="Bu sponsor için içerik henüz eklenmedi." action="İçerik ekle" onAction={onAddPublication} />}</CardContent></Card>
    <Card className="content-card"><CardHeader><div className="card-header-row"><div><CardTitle>Tahsilat, kredi ve giderler</CardTitle></div><Button variant="outline" size="sm" onClick={onAddTransaction}><Plus size={16} /> İşlem ekle</Button></div></CardHeader><CardContent>{detail?.transactions.length ? detail.transactions.map((t) => <button className="detail-list-row" key={t.id} onClick={() => onEditTransaction(t)}><div><strong>{t.note || t.publication_title || transactionLabel(t.kind)}</strong><small>{transactionLabel(t.kind)} · {trDate(t.occurred_on)}{t.publication_title ? ` · ${t.publication_title}` : ""}</small></div><b className={t.kind === "income" ? "positive" : t.kind === "credit" ? "credit" : "negative"}>{t.kind === "expense" ? "−" : "+"}{money(t.amount_minor, t.currency)}</b><ChevronRight size={16} /></button>) : <Empty text="Henüz para hareketi kaydedilmedi." action="İşlem ekle" onAction={onAddTransaction} />}</CardContent></Card>
  </div>;
}
...
function PublicationForm({ sponsors, slot, item, defaultSponsor, busy, onSubmit, onDelete }: { sponsors: Sponsor[]; slot: Slot | null; item: Publication | null; defaultSponsor: string | null; busy: boolean; onSubmit: (data: unknown) => Promise<void>; onDelete?: () => void }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { e.preventDefault(); void onSubmit({ title, planned_date: date, published_date: status === "published" ? publishedDate : null, platform, format, slot_id: lockedSlotId && date === (item?.planned_date || slot?.date) ? lockedSlotId : null, sponsor_id: sponsorId || null, fee_minor: sponsorId ? parseAmount(fee) : 0, currency, status, url: url || null, notes }); }}>
    ...
    <div className="field"><label>Sponsor</label><select value={sponsorId} onChange={(e) => setSponsorId(e.target.value)}><option value="">Organik içerik</option>{item?.sponsor_id && !sponsors.some((s) => s.id === item.sponsor_id) && <option value={item.sponsor_id}>{item.sponsor_name || "Kaldırılmış sponsor"}</option>}{sponsors.map((s) => <option value={s.id} key={s.id}>{s.name}</option>)}</select></div>
    ...
    <div className="field"><label>Yayın bağlantısı {status === "published" && "*"}</label><Input type="url" value={url} onChange={(e) => setUrl(e.target.value)} required={status === "published"} placeholder="https://" /></div><div className="field"><label>Not</label><Textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} /></div><Button type="submit" disabled={busy} className="full-button">{busy ? "Kaydediliyor…" : "İçeriği kaydet"}</Button>{item && onDelete && <button type="button" className="delete-link" disabled={busy} onClick={onDelete}><Trash2 size={15} /> İçeriği kaldır</button>}
  </form>;
}

function TransactionForm({ sponsors, item, defaultSponsor, defaultPublication, rate, busy, onSubmit, onDelete }: { sponsors: Sponsor[]; item: Transaction | null; defaultSponsor: string | null; defaultPublication: Publication | null; rate: Rate; busy: boolean; onSubmit: (data: unknown) => Promise<void>; onDelete?: () => void }) {
  ...
  const [publicationId, setPublicationId] = useState(item?.publication_id || defaultPublication?.id || "");
  ...
  const eligible = publications.filter((p) => p.status !== "cancelled");
  const selectedPublication = eligible.find((p) => p.id === publicationId) || defaultPublication;
  const amountMinor = parseAmount(amount);
  const converted = !selectedPublication || !Number.isFinite(amountMinor) ? 0 : currency === selectedPublication.currency ? amountMinor
    : !rate ? 0 : currency === "TRY" ? Math.round(amountMinor / rate.usdTry) : Math.round(amountMinor * rate.usdTry);
  const previousAllocation = item && item.publication_id === selectedPublication?.id && item.applied_currency === selectedPublication?.currency ? item.applied_minor || 0 : 0;
  const remainingBefore = selectedPublication ? Math.max(0, (selectedPublication.outstanding_minor ?? selectedPublication.fee_minor) + previousAllocation) : 0;
  const appliedMinor = selectedPublication ? Math.min(converted, remainingBefore) : 0;
  const canApply = !selectedPublication || currency === selectedPublication.currency || !!rate;
  return <form className="form-stack" onSubmit={(e) => { e.preventDefault(); void onSubmit({ kind, sponsor_id: sponsorId || null, publication_id: kind === "expense" ? null : publicationId || null, amount_minor: amountMinor, currency, applied_minor: publicationId && kind !== "expense" ? appliedMinor : null, applied_currency: publicationId && kind !== "expense" ? selectedPublication?.currency : null, occurred_on: date, note }); }}><div className="field"><label>İşlem türü</label><select value={kind} onChange={(e) => { const next = e.target.value as Transaction["kind"]; setKind(next); if (next === "expense") setPublicationId(""); }}><option value="income">Tahsilat (gelen)</option><option value="credit">Platform kredisi</option><option value="expense">Gider (giden)</option></select></div><div className="field"><label>Sponsor {kind !== "expense" && "*"}</label><select value={sponsorId} onChange={(e) => { setSponsorId(e.target.value); setPublicationId(""); }} required={kind !== "expense"}><option value="">{kind === "expense" ? "Genel gider" : "Sponsor seç"}</option>{sponsors.map((s) => <option value={s.id} key={s.id}>{s.name}</option>)}</select></div><div className="form-row"><div className="field"><label>Hesaba gelen tutar *</label><Input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} required placeholder="0,00" /></div><div className="field"><label>Gelen para birimi</label><select value={currency} onChange={(e) => setCurrency(e.target.value as Currency)}><option value="TRY">TL</option><option value="USD">USD</option></select></div></div>{sponsorId && kind !== "expense" && <div className="field"><label>Hangi video için?</label><select value={publicationId} onChange={(e) => setPublicationId(e.target.value)}><option value="">Genel sponsor tahsilatı</option>{eligible.map((p) => <option value={p.id} key={p.id}>{p.title} · {paymentLabel(p.payment_status)} · {money(p.fee_minor, p.currency)}</option>)}</select></div>}{selectedPublication && publicationId && <div className={`payment-allocation ${canApply ? "" : "unavailable"}`}><strong>{canApply ? `${money(appliedMinor, selectedPublication.currency)} videoya işlenecek` : "Kur bilgisi alınamadı"}</strong><span>Video ücreti {money(selectedPublication.fee_minor, selectedPublication.currency)} · işlem öncesi kalan {money(remainingBefore, selectedPublication.currency)}</span>{currency !== selectedPublication.currency && rate && <small>{money(amountMinor, currency)} canlı kurla çevrilir; videoya en fazla kalan tutar işlenir.</small>}</div>}<div className="field"><label>İşlem tarihi *</label><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} required /></div><div className="field"><label>Açıklama</label><Input value={note} onChange={(e) => setNote(e.target.value)} placeholder={kind === "credit" ? "Örn. Hosting kredisi" : "Örn. Eylül video ödemesi"} /></div><Button type="submit" disabled={busy || (!!publicationId && !canApply)} className="full-button">{busy ? "Kaydediliyor…" : "Ödemeyi kaydet"}</Button>{item && onDelete && <button type="button" className="delete-link" disabled={busy} onClick={onDelete}><Trash2 size={15} /> İşlemi sil</button>}</form>;
}

function paymentLabel(status?: PaymentStatus) {
  if (status === "paid") return "Ödeme alındı";
  if (status === "partial") return "Kısmi ödeme";
  if (status === "waiting") return "Ödeme bekleniyor";
  if (status === "no_fee") return "Ücretsiz";
