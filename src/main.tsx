...
type Money = Record<Currency, { contracted: number; received: number; spent: number; outstanding: number }>;
...
type Transaction = { id: string; sponsor_id: string | null; sponsor_name?: string | null; publication_id: string | null; kind: "income" | "expense"; amount_minor: number; currency: Currency; occurred_on: string; note: string };
...
const emptyMoney: Money = { TRY: { contracted: 0, received: 0, spent: 0, outstanding: 0 }, USD: { contracted: 0, received: 0, spent: 0, outstanding: 0 } };
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      <header className="topbar"><div><h1>{headline}</h1></div><div className="topbar-actions"><span className="today-chip">{trDate(todayIstanbul())}</span><Button onClick={() => openPublication()}><Plus size={17} /> Yeni içerik</Button></div></header>
      {error && <div className="error-banner">{error}<button onClick={() => setError("")}><CloseIcon size={16} /></button></div>}
      {page === "dashboard" && <DashboardPage data={dashboard} schedule={schedule} sponsors={sponsors} rate={rate} onCalendar={() => setPage("calendar")} onSponsor={(id) => { setSelectedSponsorId(id); setPage("sponsors"); }} onAdd={() => openPublication()} />}
      {page === "calendar" && <CalendarPage schedule={schedule} onWeek={setWeek} onAdd={openPublication} />}
      {page === "sponsors" && (currentSponsor ? <SponsorDetail key={`${currentSponsor.id}-${revision}`} sponsor={currentSponsor} onBack={() => setSelectedSponsorId(null)} onEdit={() => { setError(""); setModal("sponsor"); }} onAddPublication={() => openPublication()} onAddSlotPublication={(slot) => openPublication(slot)} onAddTransaction={() => openTransaction()} onEditTransaction={openTransaction} onEditPublication={(item) => openPublication(undefined, item)} /> : <SponsorsPage sponsors={sponsors.filter((s) => s.name.toLocaleLowerCase("tr").includes(search.toLocaleLowerCase("tr")))} search={search} onSearch={setSearch} onSelect={setSelectedSponsorId} onAdd={() => { setError(""); setModal("sponsor"); }} />)}
      {page === "finance" && <FinancePage dashboard={dashboard} transactions={transactions} rate={rate} onAdd={() => openTransaction()} onEdit={openTransaction} />}
    </main>
...
function AmountCards({ data, rate }: { data: Money; rate?: Rate }) {
  ...
  return <><div className="stat-grid">{values.map(({ label, key, tone }) => <Card key={key} className={`stat-card tone-${tone}`}><CardContent><div className="stat-label">{label}</div><div className="stat-value">{money(data.TRY[key], "TRY")}</div><div className="stat-secondary">{money(data.USD[key], "USD")}</div></CardContent></Card>)}</div>{rate && <div className="rate-note">ECB {trDate(rate.date)} referans kuru: 1 USD ≈ {rate.usdTry.toLocaleString("tr-TR", { maximumFractionDigits: 4 })} TL. Anlaşılan toplam ≈ {money(data.TRY.contracted + Math.round(data.USD.contracted * rate.usdTry), "TRY")} / {money(data.USD.contracted + Math.round(data.TRY.contracted / rate.usdTry), "USD")}; tahsil edilen ≈ {money(data.TRY.received + Math.round(data.USD.received * rate.usdTry), "TRY")} / {money(data.USD.received + Math.round(data.TRY.received / rate.usdTry), "USD")}. Banka işlem kuru değildir.</div>}</>;
}

function DashboardPage({ data, schedule, sponsors, rate, onCalendar, onSponsor, onAdd }: { data: Dashboard | null; schedule: Schedule | null; sponsors: Sponsor[]; rate: Rate; onCalendar: () => void; onSponsor: (id: string) => void; onAdd: () => void }) {
  ...
  return <div className="page-stack"><div className="welcome-panel"><div><h2>Her yayın yerli yerinde.</h2><p>Haftalık içerik düzenin, sponsorlukların ve gelirlerin tek bakışta.</p><Button onClick={onCalendar} variant="outline">Takvime git <ChevronRight size={16} /></Button></div><div className="welcome-numbers"><div><strong>3</strong><span>uzun video</span></div><div><strong>4</strong><span>YouTube Short</span></div><div><strong>25</strong><span>haftalık yayın yuvası</span></div></div></div>
    <div className="section-heading"><div><h2>Rakamlarla genel durum</h2></div></div><AmountCards data={data?.money || emptyMoney} rate={rate || undefined} />
...
function SponsorsPage({ sponsors, search, onSearch, onSelect, onAdd }: { sponsors: Sponsor[]; search: string; onSearch: (v: string) => void; onSelect: (id: string) => void; onAdd: () => void }) {
  return <div className="page-stack"><div className="section-heading"><div><h2>Tüm sponsorlar</h2><p>Marka bazında içerikleri, ödemeleri ve giderleri izle.</p></div><Button onClick={onAdd}><Plus size={17} /> Sponsor ekle</Button></div><div className="search-box"><Search size={18} /><Input placeholder="Sponsor ara…" value={search} onChange={(e) => onSearch(e.target.value)} /></div><div className="sponsor-grid">{sponsors.map((s) => <button className="sponsor-tile" key={s.id} onClick={() => onSelect(s.id)}><div className="tile-top"><SponsorLogo sponsor={s} /><ChevronRight size={18} /></div><h3>{s.name}</h3><div className="tile-counts"><span>{s.published || 0} yayında</span><span>{s.planned || 0} bekliyor</span></div><div className="tile-money"><strong>{money(s.money?.TRY.received || 0, "TRY")}</strong><small>+ {money(s.money?.USD.received || 0, "USD")} tahsil edildi</small></div></button>)}</div>{!sponsors.length && <Empty text="Aradığın sponsor bulunamadı." />}</div>;
}

function SponsorDetail({ sponsor, onBack, onEdit, onAddPublication, onAddSlotPublication, onAddTransaction, onEditPublication, onEditTransaction }: { sponsor: Sponsor; onBack: () => void; onEdit: () => void; onAddPublication: () => void; onAddSlotPublication: (slot: Slot) => void; onAddTransaction: () => void; onEditPublication: (item: Publication) => void; onEditTransaction: (item: Transaction) => void }) {
  ...
  return <div className="page-stack"><div className="detail-heading"><button className="back-link" onClick={onBack}><ChevronLeft size={16} /> Sponsorlar</button><div className="detail-title"><SponsorLogo sponsor={sponsor} /><div><h2>{sponsor.name}</h2>{sponsor.website_url && <a href={sponsor.website_url} target="_blank" rel="noreferrer">Web sitesini aç ↗</a>}</div><Button variant="outline" size="sm" onClick={onEdit}>Düzenle</Button></div>{sponsor.notes && <p className="sponsor-note">{sponsor.notes}</p>}</div>
    <div className="count-strip"><div><strong>{detail?.publications.filter((p) => p.status !== "cancelled" && ["long", "short", "reel", "tiktok", "teaser"].includes(p.format)).length || 0}</strong><span>Anlaşılan video</span></div><div><strong>{detail?.published || 0}</strong><span>Yayımlanan</span></div><div><strong>{detail?.planned || 0}</strong><span>Bekleyen</span></div></div><AmountCards data={detail?.money || emptyMoney} />
    ...
    <Card className="content-card"><CardHeader><div className="card-header-row"><div><CardTitle>Tahsilat ve giderler</CardTitle></div><Button variant="outline" size="sm" onClick={onAddTransaction}><Plus size={16} /> İşlem ekle</Button></div></CardHeader><CardContent>{detail?.transactions.length ? detail.transactions.map((t) => <button className="detail-list-row" key={t.id} onClick={() => onEditTransaction(t)}><div><strong>{t.note || (t.kind === "income" ? "Tahsilat" : "Gider")}</strong><small>{trDate(t.occurred_on)}</small></div><b className={t.kind === "income" ? "positive" : "negative"}>{t.kind === "income" ? "+" : "−"}{money(t.amount_minor, t.currency)}</b><ChevronRight size={16} /></button>) : <Empty text="Henüz para hareketi kaydedilmedi." action="İşlem ekle" onAction={onAddTransaction} />}</CardContent></Card>
  </div>;
}

function FinancePage({ dashboard, transactions, rate, onAdd, onEdit }: { dashboard: Dashboard | null; transactions: Transaction[]; rate: Rate; onAdd: () => void; onEdit: (item: Transaction) => void }) {
  return <div className="page-stack"><div className="section-heading"><div><h2>Finansal özet</h2><p>Kararlaştırılan ücretler ve gerçek para hareketleri ayrı izlenir.</p></div><Button onClick={onAdd}><Plus size={17} /> İşlem ekle</Button></div><AmountCards data={dashboard?.money || emptyMoney} rate={rate || undefined} /><Card className="content-card"><CardHeader><CardTitle>Tahsilatlar ve giderler</CardTitle></CardHeader><CardContent>{transactions.length ? transactions.map((t) => <button className="finance-row finance-button" key={t.id} onClick={() => onEdit(t)}><span className={`transaction-icon ${t.kind}`}>{t.kind === "income" ? "↓" : "↑"}</span><div className="row-main"><strong>{t.sponsor_name || "Genel gider"}</strong><small>{t.note || (t.kind === "income" ? "Tahsilat" : "Gider")}</small></div><span className="row-date">{trDate(t.occurred_on)}</span><b className={t.kind === "income" ? "positive" : "negative"}>{t.kind === "income" ? "+" : "−"}{money(t.amount_minor, t.currency)}</b></button>) : <Empty text="Henüz tahsilat veya gider kaydı yok." action="İşlem ekle" onAction={onAdd} />}</CardContent></Card></div>;
}
...
function PublicationForm({ sponsors, slot, item, defaultSponsor, busy, onSubmit }: { sponsors: Sponsor[]; slot: Slot | null; item: Publication | null; defaultSponsor: string | null; busy: boolean; onSubmit: (data: unknown) => Promise<void> }) {
  ...
  return <form className="form-stack" onSubmit={(e) => { e.preventDefault(); void onSubmit({ title, planned_date: date, published_date: status === "published" ? publishedDate : null, platform, format, slot_id: lockedSlotId && date === (item?.planned_date || slot?.date) ? lockedSlotId : null, sponsor_id: sponsorId || null, fee_minor: sponsorId ? parseAmount(fee) : 0, currency, status, url: url || null, notes }); }}>
    ...
    {lockedSlotId && date !== (item?.planned_date || slot?.date) && <div className="date-warning">Tarih değişince içerik bu sabit haftalık yuvadan ayrılır. Yeni günü takvimde ek yayın olarak görünür.</div>}
...
function TransactionForm({ sponsors, item, defaultSponsor, busy, onSubmit }: { sponsors: Sponsor[]; item: Transaction | null; defaultSponsor: string | null; busy: boolean; onSubmit: (data: unknown) => Promise<void> }) {
  const [kind, setKind] = useState<"income" | "expense">(item?.kind || "income"); const [sponsorId, setSponsorId] = useState(item?.sponsor_id || defaultSponsor || "");
